    ...
    $scope.getDetail = function() {
      $http.get("http://api.ifest-uajy.com/v1/hackfest/"+$scope.idTeam).then(function (response) {
        ...
        if ($scope.dataTeam.proposal != 0) {
          ...
          var row = angular.element('<tr class="details"><td><a href="http://api.ifest-uajy.com/storage/media/{{ dataTeam.proposal_name }}" target="_blank">Lihat</a></td><td><a ng-show="dataTeam.receipt != 0" href="http://api.ifest-uajy.com/storage/media/{{ dataTeam.receipt_name }}" target="_blank">Lihat</a><button ng-show="dataTeam.receipt == 0" type="file" ngf-select="uploadReceipt($file, $invalidFiles, dataTeam.id)" ngf-max-size="10MB" class="btn">Unggah</button> <span ng-show="dataTeam.receipt == 0" class="receipt-info {{ dataTeam.id }}">Pilih file untuk diunggah</span></td><td><span ng-show="dataTeam.status == 0">Menunggu verifikasi</span><span ng-show="dataTeam.status == 1">Lolos</span></td><td><button ng-show="dataTeam.receipt == 0" ng-click="updateDetail(dataTeam.id)" type="button" class="btn update-detail {{ dataTeam.id }}">Simpan</button><button ng-show="dataTeam.status == 0" ng-click="destroyDetail(dataTeam.id)" type="button" class="btn delete-detail {{ dataTeam.id }}">Hapus</button></td></tr>');
          ...
          var row = angular.element('<tr class="new-details"><td><button type="file" ngf-select="uploadProposal($file, $invalidFiles)" accept="image/*" ngf-max-size="10MB" class="btn">Unggah</button> <span>{{ infoProposal }}</span></td><td></td><td></td><td><button ng-click="addDetail()" type="button" class="btn">{{ btnSave }}</button></td></tr>');
